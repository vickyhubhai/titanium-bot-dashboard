import { t as SiteShell } from "./SiteShell-AqHnm6be.js";
import { t as PageHeader } from "./PageHeader-CMylNLfZ.js";
import { Link } from "@tanstack/react-router";
import { jsx, jsxs } from "react/jsx-runtime";
//#region src/routes/partners.tsx?tsr-split=component
var tiers = [
	{
		n: "Referrer",
		p: "20% revshare",
		d: "For consultants and creators who introduce communities to Titanium Security.",
		perks: [
			"Unique referral link",
			"Quarterly payouts",
			"Co-marketing on request"
		]
	},
	{
		n: "Implementation",
		p: "Listed in directory",
		d: "For agencies who configure and operate Titanium Security on behalf of large guilds.",
		perks: [
			"Multi-guild dashboard",
			"Priority engineering review",
			"Joint case studies"
		],
		featured: true
	},
	{
		n: "Technology",
		p: "API + design partnership",
		d: "For platforms that want to embed Titanium Security data inside their own surfaces.",
		perks: [
			"Early API access",
			"Design-partner roadmap input",
			"Shared incident response"
		]
	}
];
function PartnersPage() {
	return /* @__PURE__ */ jsxs(SiteShell, { children: [/* @__PURE__ */ jsx(PageHeader, {
		eyebrow: "Partners",
		title: "Build with us, for the communities that matter.",
		sub: "A small, curated partner program for the people who already do the work. No badges, no leaderboards — just leverage."
	}), /* @__PURE__ */ jsx("section", {
		className: "mx-auto grid max-w-6xl gap-5 px-6 pb-16 md:grid-cols-3",
		children: tiers.map((t) => /* @__PURE__ */ jsxs("div", {
			className: `glass relative rounded-2xl p-6 ${t.featured ? "ring-1 ring-brand/60" : ""}`,
			children: [
				t.featured && /* @__PURE__ */ jsx("div", {
					className: "absolute -top-3 left-6 rounded-full bg-gradient-to-r from-brand to-brand-glow px-3 py-1 font-mono text-[10px] uppercase tracking-widest text-brand-foreground",
					children: "Most active"
				}),
				/* @__PURE__ */ jsx("h2", {
					className: "font-display text-xl font-bold",
					children: t.n
				}),
				/* @__PURE__ */ jsx("div", {
					className: "mt-1 font-mono text-xs uppercase tracking-widest text-brand",
					children: t.p
				}),
				/* @__PURE__ */ jsx("p", {
					className: "mt-3 text-sm text-muted-foreground",
					children: t.d
				}),
				/* @__PURE__ */ jsx("ul", {
					className: "mt-5 space-y-2 text-sm",
					children: t.perks.map((p) => /* @__PURE__ */ jsxs("li", {
						className: "flex items-center gap-2 text-foreground/90",
						children: [/* @__PURE__ */ jsx("span", { className: "h-1.5 w-1.5 rounded-full bg-brand" }), p]
					}, p))
				}),
				/* @__PURE__ */ jsx(Link, {
					to: "/contact",
					className: "mt-6 inline-flex h-10 w-full items-center justify-center rounded-xl border border-border/60 bg-surface/40 px-4 text-sm font-semibold transition-colors hover:bg-surface/70",
					children: "Apply"
				})
			]
		}, t.n))
	})] });
}
//#endregion
export { PartnersPage as component };
